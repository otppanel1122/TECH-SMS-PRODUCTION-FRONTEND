  ...
  const loadClients = async () => {
    ...
    try {
      ...
      if (response.data && response.data.success) {
        ...
        const errorMsg = response.data?.error || 'Failed to load clients';
    ...
    <div className="space-y-4 sm:space-y-6">
      <div className="glass-card px-4 sm:px-6 py-4">
        <p className="text-sm text-slate-500 dark:text-slate-400">
          Total: <span className="text-slate-900 dark:text-white font-medium">{clients.length}</span> clients
        </p>
      ...
      <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between">
        <div className="flex flex-wrap items-center gap-3 w-full sm:w-auto">
          ...
          <select 
            ...
            className="select-field w-20 sm:w-24 text-sm"
          >
              ...
              {paginatedClients.length > 0 ? paginatedClients.map((client) => (
                <tr key={client.id}>
                  <td className="px-3 sm:px-4 py-3">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <div className="font-medium text-slate-900 dark:text-white text-sm">{client.name}</div>
                        <div className="text-xs text-slate-500 sm:hidden">{client.email}</div>
                      </div>
                    </div>
                  </td>
                ...
                <tr>
                  <td colSpan={7} className="px-4 py-8 text-center text-slate-500 dark:text-slate-400">
                    {error ? 'Error loading clients. Please refresh.' : 'No clients found'}
                  </td>
                </tr>
              )}
            </tbody>
      ...
      {totalPages > 1 && (
        <div className="flex flex-col sm:flex-row items-center justify-between gap-3">
          <div className="text-xs sm:text-sm text-slate-500 dark:text-slate-400">
            Showing {startIndex + 1} to {Math.min(startIndex + pageSize, filteredClients.length)} of {filteredClients.length}
          </div>
